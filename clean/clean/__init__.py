...
def load_speed_from_csv(path):
    ...
    for filename in tqdm(all_files):
        ...
        df.columns = ['Id', 'Speed', 'TravelTime',
                      'Status', 'DataAsOf', 'linkId']
        if df['Speed'].dtype != np.float64:
            cond = ((df['Speed'].str.contains('Speed')) |
                    (df['Speed'].str.contains('Bronx')))
            df[cond] = np.nan
...
def load_speed_from_table():
    ...
    df = pd.read_table(
        'http://207.251.86.229/nyc-links-cams/LinkSpeedQuery.txt', parse_dates=['DataAsOf'])
...
def downsample_sensors(df, freq):
    ...
    for group in df.groupby('linkId'):
        ...
        df_i = df_tem.rename(columns={'Speed': df_tem['linkId'][0]})[
            df_tem['linkId'][0]]
...
def clean_median_speed(year=2019,
                       path='*',
                       boro_sel=['Manhattan', 'Staten Island',
