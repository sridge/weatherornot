...
def normalize(data_in):
    ...
    mn = pd.read_csv(f'mean_{ampm}.csv',index_col=0).T
    std = pd.read_csv(f'std_{ampm}.csv',index_col=0).T
...
def speed_forecast_2h(boro_sel = ['Manhattan','Staten Island','Queens','Bronx','Brooklyn'],freq = '15min'):
    ...
    df = clean.subset_speed_data(df,boro_sel,link_id_path='linkIds.csv')
