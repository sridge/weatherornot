...
def synthetic_weather_forecast(df_weather, window):
    ...
    df_weather_pred = df_weather_pred.set_index(
        df_weather.index - pd.Timedelta(window))
...
def climacell_json_to_df(json_out, freq):
    ...
    df_weather['observation_time'] = pd.to_datetime(
        df_weather['observation_time']) - pd.Timedelta('30min')
...
def format_data_in_2h(median_speed, df_weather, df_weather_pred):
    ...
    df_weather_pred_tem = df_weather_pred[df_weather_pred.index.isin(
        data_in.index)]
...
def speed_forecast_2h(boro_sel=['Manhattan', 'Staten Island', 'Queens', 'Bronx', 'Brooklyn'],
    ...
    median_speed = median_speed.tail(10)
    df_weather, df_weather_pred = get_weather_data(
        freq, end_time=time_nearest_15min())
    ...
    ampm, data_in = format_data_in_2h(
        median_speed, df_weather, df_weather_pred)
...
def alert_hours(df_forecast, thresh=0):
    ...
    df_forecast['weather_anom'] = df_forecast['weather'] - \
        df_forecast['no_weather']
...
def send_alerts(hours,
    ...
    with smtplib.SMTP_SSL("smtp.gmail.com", port, context=context) as server:
        ...
        for hour in hours:
            ...
            df_users_alert = df_users[(df_users['enter_nyc'] == hour) | (
                df_users['leave_nyc'] == hour)]
